...
class Template
{
    ...
    protected $config = [
        ...
        'cache_path'         => '',
        'cache_suffix'       => 'sw', // 默认模板缓存后缀
        'tpl_deny_func_list' => 'echo,exit', // 模板引擎禁用函数
        'tpl_deny_php'       => false, // 默认模板引擎是否禁用PHP原生代码
        'label_tag'          => '{', // 模板引擎普通标签开始标记
        'label_tagend'            => '}', // 模板引擎普通标签结束标记
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->config['label_tag']    = preg_quote($this->config['label_tag'], '/');
        $this->config['label_tagend']      = preg_quote($this->config['label_tagend'], '/');

        // 初始化模板编译存储器
        $type  = $this->config['compile_type'] ? $this->config['compile_type'] : 'File';
        $class = false !== strpos($type, '\\') ? $type : '\\sower\\template\\driver\\' . ucwords($type);
    ...
    public function parseTagLib(string $tagLib, string &$content, bool $hide = false): void
    {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = '\\sower\\template\\taglib\\' . ucwords($tagLib);
        }
        ...
        if (preg_match_all($regex, $content, $matches, PREG_SET_ORDER)) {
            foreach ($matches as $match) {
                ...
                switch ($flag) {
                    ...
                    default:
                        // 未识别的标签直接返回
                        $str = $this->config['label_tag'] . $str . $this->config['label_tagend'];
                        break;
                }
                ...
                if (isset($_varParseList[$match[0]])) {
                    ...
                    if (strpos($match[0], '.')) {
                        ...
                        if (isset($this->extend[$first])) {
                            ...
                            // 所有以Sower.打头的以特殊变量对待 无需模板赋值就可以输出
                            $parseStr = $this->parseSowerVar($vars);
                        } else {
    ...
    /**
     * 特殊模板变量解析
     * 格式 以 $Sower. 打头的变量属于特殊模板变量
     * @access public
     * @param  array $vars 变量数组
     * @return string
     */
    public function parseSowerVar(array $vars): string
    {
        ...
        if ($vars) {
            ...
            switch ($type) {
                case 'NOW':
                    ...
                    break;
                case 'LDELIM':
                    $parseStr = '\'' . ltrim($this->config['label_tag'], '\\') . '\'';
                    break;
                case 'RDELIM':
                    $parseStr = '\'' . ltrim($this->config['label_tagend'], '\\') . '\'';
                    break;
                default:
                    if (defined($type)) {
                        $parseStr = $type;
    ...
    private function getRegex(string $tagName): string
    {
        $regex = '';
        if ('tag' == $tagName) {
            $begin = $this->config['label_tag'];
            $end   = $this->config['label_tagend'];
